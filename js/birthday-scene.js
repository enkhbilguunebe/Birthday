...
const ambient=document.querySelector('#ambientMusic');
...
renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<800?1.45:2));
...
const key=new THREE.SpotLight(0xffd8bd,85,35,Math.PI/5,.4,1.4);key.position.set(-5,10,7);key.target.position.set(0,0,0);key.castShadow=true;key.shadow.mapSize.set(1024,1024);scene.add(key,key.target);
const purpleLight=new THREE.PointLight(0xa85cff,28,24,2);purpleLight.position.set(5,6,-2);scene.add(purpleLight);

function roundedBox(w,h,d,color,rough=.6,metal=.02){const g=new THREE.BoxGeometry(w,h,d,6,3,3);const m=new THREE.MeshStandardMaterial({color,roughness:rough,metalness:metal});const mesh=new THREE.Mesh(g,m);mesh.castShadow=true;mesh.receiveShadow=true;return mesh}
function addStars(){const count=950,p=new Float32Array(count*3);for(let i=0;i<count;i++){const r=80+Math.random()*80,a=Math.random()*Math.PI*2,y=12+Math.random()*70;p[i*3]=Math.cos(a)*r;p[i*3+1]=y;p[i*3+2]=Math.sin(a)*r}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(p,3));scene.add(new THREE.Points(g,new THREE.PointsMaterial({color:0xe7ddff,size:.28,transparent:true,opacity:.8,sizeAttenuation:true})))}
addStars();

// Night sky dome
const sky=new THREE.Mesh(new THREE.SphereGeometry(180,32,20),new THREE.MeshBasicMaterial({color:0x07030e,side:THREE.BackSide}));scene.add(sky);
...
for(let i=0;i<95;i++){
  const angle=(i/95)*Math.PI*2; const radius=35+Math.random()*10; const h=4+Math.random()*16; const w=.9+Math.random()*2.2;
...
// Table and cloth
const tableTop=new THREE.Mesh(new THREE.CylinderGeometry(6.4,6.4,.42,64),new THREE.MeshStandardMaterial({color:0x5a352d,roughness:.72}));tableTop.position.y=-.05;tableTop.castShadow=true;tableTop.receiveShadow=true;scene.add(tableTop);
...
const cloth=new THREE.Mesh(new THREE.CylinderGeometry(6.18,6.18,.12,64),new THREE.MeshStandardMaterial({map:clothTex,roughness:.88}));cloth.position.y=.2;cloth.receiveShadow=true;scene.add(cloth);

// Cake stand
const stand=new THREE.Mesh(new THREE.CylinderGeometry(2.62,2.9,.22,64),new THREE.MeshPhysicalMaterial({color:0xf3e9ef,roughness:.18,metalness:.05,clearcoat:.75}));stand.position.y=.47;stand.castShadow=true;stand.receiveShadow=true;scene.add(stand);
...
// Cocoa crumbs
for(let i=0;i<150;i++){const crumb=new THREE.Mesh(new THREE.SphereGeometry(.025+Math.random()*.025,5,4),new THREE.MeshStandardMaterial({color:0x2d140d,roughness:1}));crumb.position.set((Math.random()-.5)*4.05,2.46,(Math.random()-.5)*3.5);cake.add(crumb)}
// plaque
...
for(let i=0;i<19;i++){
  ...
  const light=new THREE.PointLight(0xff9a35,.42,2.1,2);light.position.copy(flame.position);candleGroup.add(light);
...
// Ambient city glows
for(let i=0;i<40;i++){const light=new THREE.PointLight(i%2?0x7d7cff:0xff4f9b,.45,5,2);const a=Math.random()*Math.PI*2,r=24+Math.random()*14;light.position.set(Math.cos(a)*r,2+Math.random()*12,Math.sin(a)*r);scene.add(light)}
progress(84);

const clock=new THREE.Clock();let paused=false;
function animate(){requestAnimationFrame(animate);if(paused)return;const t=clock.getElapsedTime();candles.forEach(c=>{if(!c.out){const k=.88+Math.sin(t*15+c.phase)*.13;c.flame.scale.set(.75*k,1.55*k,.75*k);c.light.intensity=.34+Math.random()*.2}});skyline.rotation.y=Math.sin(t*.025)*.015;controls.update();renderer.render(scene,camera)}
animate();
...
document.querySelector('#closeMessage').onclick=()=>{document.querySelector('#birthdayMessage').classList.add('closed');ambient.volume=.28;ambient.play().catch(()=>toast('Add assets/music/background.mp3 to enable music.'))};
document.querySelector('#musicToggle').onclick=()=>{const active=!ambient.paused||!birthday.paused;if(active){ambient.pause();birthday.pause();toast('Music muted.')}else{(celebration?birthday:ambient).play().catch(()=>toast('Music file has not been added yet.'))}};
...
function launchEffects(){for(let i=0;i<140;i++){const e=document.createElement('i');e.className='confetti';e.style.left=Math.random()*100+'vw';e.style.background=`hsl(${Math.random()*360} 90% 70%)`;e.style.setProperty('--duration',2.4+Math.random()*3+'s');e.style.setProperty('--drift',Math.random()*240-120+'px');effects.appendChild(e);setTimeout(()=>e.remove(),6000)}for(let i=0;i<16;i++){const b=document.createElement('i');b.className='balloon';b.style.left=Math.random()*95+'vw';b.style.filter=`hue-rotate(${Math.random()*220}deg)`;b.style.setProperty('--duration',5+Math.random()*4+'s');b.style.setProperty('--drift',Math.random()*180-90+'px');effects.appendChild(b);setTimeout(()=>b.remove(),9500)}for(let i=0;i<10;i++)setTimeout(()=>{const f=document.createElement('i');f.className='firework';f.style.left=12+Math.random()*76+'vw';f.style.top=8+Math.random()*48+'vh';effects.appendChild(f);setTimeout(()=>f.remove(),1500)},i*240)}
...
addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<800?1.45:2));renderer.setSize(innerWidth,innerHeight)});
